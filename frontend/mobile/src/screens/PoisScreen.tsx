...
export default function PoisScreen() {
  ...
  const fetchPois = async (
    ...
    try {
      ...
      // For global filters (recent/reports), don't send lat/lng to backend
      const isGlobal = orderBy === 'recent' || orderBy === 'reports';
      const latParam = isGlobal ? undefined : center.lat;
      const lngParam = isGlobal ? undefined : center.lng;

      try { console.log('[fetchPois] request', { lat: latParam, lng: lngParam, radius, type: requestedType, page: pageToLoad, limit, q: searchQuery, search: isSearching, orderBy, isGlobal }); } catch { }
      const { pois: apiPois, pagination } = isSearching
        ...
        : await getPois({ lat: latParam, lng: lngParam, radius, type: requestedType, include_contributions: true, page: pageToLoad, limit, forceRefresh: !!forceRefresh });
      ...
      // Ensure distance is present also for search results (only when not global)
      if (!isGlobal) {
