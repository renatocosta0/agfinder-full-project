...
/**
 * Find POIs globally (no location filter) ordered by recent or reports
 * Used for "Recent updates" and "Most reports" global filters
 * 
 * @param {string} type - Tipo de POI ('atm'|'gasstation')
 * @param {string} sortBy - Ordenação ('recent' | 'reports')
 * @param {Object} options - Opções de paginação
 * @returns {Promise<Object>} Resultados com POIs e metadados
 */
async function findPOIsGlobal(type, sortBy = 'recent', options = {}) {
  const {
    page = 1,
    limit = 20,
  ...
  // Cache key
  const cacheKey = `pois:global:${type || 'all'}:${sortBy}:p${page}:l${limit}`;
  const cachedResults = await cacheService.get(cacheKey);
  if (cachedResults && !options.forceRefresh) {
    logger.info(`Cache hit for global POIs: ${cacheKey}`);
    return { ...cachedResults, source: 'cache' };
  }

  // Build where clause (only type, no location)
  const whereClause = {};
  if (type) {
    whereClause.poi_type = type;
  }
  ...
  // Calculate total_interactions for sorting
  const poisWithMetrics = pois.map(poi => {
    const current = Array.isArray(poi.contributions) && poi.contributions.length > 0
      ? (poi.contributions.find(c => c.is_current) || poi.contributions[0])
      : null;
    ...
    const createdAt = current ? current.created_at : null;
    return {
      poi,
      totalInteractions,
      reportsCount,
  ...
  if (sortBy === 'recent') {
    ...
    poisWithMetrics.sort((a, b) => b.reportsCount - a.reportsCount);
  }

  // Paginate
  const totalCount = poisWithMetrics.length;
  const pageItems = poisWithMetrics.slice(offset, offset + limit);

  const results = pageItems.map(({ poi, totalInteractions, createdAt }) => ({
    id: poi.id,
    google_place_id: poi.google_place_id,
    name: poi.name,
    type: poi.poi_type,
    ...
    distance_km: undefined, // No distance for global results
    address: poi.address,
    last_sync_at: poi.last_sync_at,
    google_data: poi.google_data || null,
    contributions: includeContributions && poi.contributions ? poi.contributions.map(c => ({
  ...
  // Cache for shorter TTL (5 min for global results)
